...
class ACTLossHead(nn.Module):
    ...
    def forward(
        ...
        **model_kwargs,
        ...
        # Value prediction loss - Use MSE for regression, not cross-entropy
        value_loss = 0
        if "value_logits" in outputs and "value_targets" in new_carry.current_data:
            ...
            valid_value_mask = ~torch.isnan(value_targets.float())  # Ensure float for NaN check
            
            if valid_value_mask.any():
                value_logits_valid = value_logits[valid_value_mask]
                value_targets_valid = value_targets[valid_value_mask].float()  # Convert to float
