...
from HRM_model.models.common import trunc_normal_init_
from HRM_model.models.layers import rms_norm, SwiGLU, Attention, RotaryEmbedding, CosSin, CastedEmbedding, CastedLinear
from HRM_model.models.sparse_embedding import CastedSparseEmbedding
...
class HierarchicalReasoningModel_ACTV1_Inner(nn.Module):
    ...
    def _input_embeddings(self, input: torch.Tensor, puzzle_identifiers: torch.Tensor):
        ...
        if self.config.pos_encodings == "learned" and not self.config.use_chess_tokenization:
            ...
            board_positions = embedding[:, self.puzzle_emb_len:, :]  # [batch, seq_len, hidden_size]
            board_positions = board_positions + self.pos_encoding_2d.unsqueeze(0)  # Add 2D pos encodings
            
            if self.puzzle_emb_len > 0:
                embedding = torch.cat([embedding[:, :self.puzzle_emb_len, :], board_positions], dim=1)
            else:
