...
def _sample_batch_simple(rng: np.random.Generator, puzzle_indices: np.ndarray, global_batch_size: int, num_puzzles: int):
    # Pack examples into a full batch by directly sampling puzzles (no groups)
    batch = []
    batch_puzzle_indices = []
    current_size = 0

    while current_size < global_batch_size:
        # Directly sample a puzzle ID from available puzzles
        puzzle_id = rng.integers(0, num_puzzles)
    ...
    return np.concatenate(batch), np.concatenate(batch_puzzle_indices)
...
class PuzzleDataset(IterableDataset):
    ...
    def _iter_train(self):
        for set_name, dataset in self._data.items():  # type: ignore
            ...
            num_groups = max(1, dataset["group_indices"].size - 1)
